...
// share-slugs.json lives one level up, OUTSIDE the published git repo
// (moved 2026-09-02) — it's build-only bookkeeping (opaque public-slug ->
// real trip-id mapping) never read by any live page, same "keep
// build-only bookkeeping outside the public tree" pattern as Student
// Portal's portal_passwords.json. trips.json is different: it moved back
// INSIDE the repo 2026-09-07 once the root landing page that reads it was
// restored (safe now that Pangolin gates the whole trip.qloof.cc origin)
// — see DEV_NOTES.md "Root index.html deleted" / "Password gate".

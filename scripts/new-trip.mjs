...
// need updating to match. It also appends the new trip to trips.json (kept
// outside the published repo, build-only bookkeeping — no live page reads
// it since the trip-listing landing page was deleted, see DEV_NOTES.md).
//
// Usage:
...
const newCall = `initTripDashboard({
    tripId: ${JSON.stringify(slug)},
    tripLabel: ${JSON.stringify(label)},
    tripSeed: ${seedJson},
  });`;
html = html.replace(callRe, newCall);
